...
依赖：tie 语言（自举生产工具链）；本库模块一完成即被引擎「模块一（绘制表 IR）」端验证用（可先行联调）；资源句柄依赖引擎 §9 的资源管理接口（先行定义接口契约）。

---

## 10. 风险与边界 / Risks & Non-scope

* **API 与 tie 语法演进耦合** → 缓解：本设计以"数据模型形状"为准，示例语法标注待定稿；库先以稳定数据模型落地
* **与 UI 层差分协议对接晚** → 缓解：模块二即定稿增量段与 key 前缀协议（与 UI 库 p.9.3.2 并行协商）
* **资源生命周期跨层复杂度** → 缓解：句柄集中在资源模块，引擎 LRU 换出对上层透明
* **本期不做**：事件/输入（事件轴独立文档）、无障碍、平台窗口壳（p.9.3.1 运行时底座）、高级效果库（模糊/阴影之外的扩展集，另行规划）
