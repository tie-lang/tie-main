...
| 项 | 语义 | tiec 落点 | 改动量 |
|---|---|---|---|
| **L1 可见性强制化** | namespace 内非 pub 跨 ns 不可见（符号解析报未定义） | sstate（pub 标记）→ sinfer（校验）→ diagcode | 小 |
| **L2 pub const** | 常量进符号表、跨文件可见 | sstate + consteval（基建已有） | 小 |
| **L3 import 语义升级** | 文本内联 → 命名空间绑定（`import middle.passes` 仅限定名可达，顶层不互相污染）；双轨过渡（per-file 模块模式标记） | mexpand + parser + sstate | 中 |
| **L4 模块即编译单元** | 模块 IR + 符号表可序列化（tieir_ser 基建现成）+ 模块级缓存（联动 p.9.15）+ ABI 哈希 | tieir_ser + driver/cache | 大 |
| **L5 编译期分发表（可选）** | 服务 builtin_expr 表驱动：宏/编译期生成名字→处理函数分发（mexpand 宏基建可承载） | mexpand 或 sinfer | 中 |
