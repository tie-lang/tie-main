...
> 本文档分析 GPU/NPU/SIMD/多核加速 tie 编译与 tdb 的可行性，及定稿路线。
> EN: This document analyzes the feasibility of using GPU/NPU/SIMD/multi-core to accelerate tie compilation and tdb, and the finalized roadmap.
> 决策汇总：
> EN: Decision summary:
> **编译器不做 GPU 加速**（串行依赖负载不适合）；**tdb 向量检索做 GPU**
> EN: **The compiler does not use GPU acceleration** (unsuitable for serial-dependency workloads); **tdb vector retrieval uses GPU**
...
> tdb（vecsearch Flat 索引）、ext/ml、序列化规范（zd）。
> EN: tdb (vecsearch Flat index), ext/ml, serialization spec (zd).
...
## 3. tdb 加速（真正的 GPU 场景）
*EN: 3. tdb Acceleration (The Real GPU Scenario)*
...
EN: For tdb operations, vector retrieval (vecsearch Flat) is the standout GPU ideal (data-parallel), while zd encode/decode and sort/filter are 🟡 (CPU fine) and transactions/indexes/query planning are ❌ (latency-sensitive).
...
- tdb 的 vecsearch Flat 精确索引 = 暴力全量计算——**GPU 天然加速**
  EN: tdb's vecsearch Flat exact index = brute-force full computation — **naturally accelerated on GPU**.
...
// tdb 查询路径（编译期/运行期选择，与 P4b backend 同思路）：
...
| 场景 | NPU 适配 | 说明 |
| --- | --- | --- |
| **embedding 模型推理**（文本→向量） | ✅✅ 主场景 | 语义检索前置：tdb 向量入库前文本→向量 |
...
- tdb 从"向量数据库"升级为**语义检索数据库**（embedding + 检索全本地）
  EN: tdb upgrades from a "vector database" to a **semantic-retrieval database** (embedding + retrieval, fully local).
...
| tdb 加速 | GPU 向量检索（trm.gpu，FAISS 同款，10-50x） | CPU 并行 |
...
EN: Decision matrix: no compiler GPU acceleration; tdb gets GPU vector retrieval; NPU targets the Intel consumer-CPU built-in NPU for embedding inference; implementation is extern bridge + auto-fallback; priority is P1 SIMD/parallel → P2 GPU retrieval → P3 NPU inference.

## 9. 未决问题
*EN: 9. Open Questions*

1. **trm.gpu 域形态**：独立域（trm.gpu）还是 tdb 内部 gpu.tie——
   倾向 trm.gpu（可插拔，与 trm.npu 并列）
   EN: **Form of the trm.gpu domain**: an independent domain (trm.gpu) or gpu.tie inside tdb — leaning toward trm.gpu (pluggable, parallel to trm.npu).
...
4. **NPU 无感知设计**：tdb 是否完全隐藏 NPU（embedding API 自动选设备）——
   倾向隐藏（用户只调 `tiedb.embed(text)`）
   EN: **NPU-transparent design**: whether tdb fully hides the NPU (embedding API auto-selects the device) — leaning toward hidden (users only call `tiedb.embed(text)`).
