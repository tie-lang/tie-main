...
- `spawn`/`yield`/`collect` 成为 tiec 完整内置原语（三处注册：is\_builtin\_name / builtin\_call / ensure\_builtins 主表 73→76）。

- 静态链接：`g_used_trmlite` → `link_exe` 追加 `trm_lite.a`（`TIE_TRM_LITE_LIB` 环境变量 → 相对默认路径）；`tig_trmlite_call` 直发 extern\_call 不置 tie\_interp。
...
- actor 默认由简单执行体承载（替换 1:1 线程默认）：irgen\_rt 每条消息 = `trm_lite_sched$spawn_task` 任务；yield 排空 + actor 等待循环 + 同步 RPC；actor 与 import trm-lite 混用 → 编译期报错。

- 闭包字面量 spawn 端到端通过（含捕获、void 返回、嵌套 spawn）。

- 汇总库 `core/runtime/tl_runtime.tie`（import sched+gc → trm\_lite.a）。
...
2. **复杂形态完整实现**（用户定调，超越设计 §8 的最小可靠子集）：
   ...
   - 抢占能力与 trm 路线 B 对齐。
3. **验收闭环**：多执行体分配/回收 + 消息传收 demo；简单 vs 复杂行为一致；m6\_actor 零回归。
...
- **p.6.5.10 回归与对比验收**：m6\_actor 零回归、路线 A/B 不受影响、简单 vs 复杂行为一致（输出/退出码）。
...
- **p.6.5.6 拍板（2026-09-02，设计 §9 待决项 3 定案）——「任务 env 即根」**：
  ...
  显式登记 + 写屏障（set\_ref 黑→白 置灰、循环中晚到根置灰）维护；运行时其余
  ...
  （该精度损失已由探针 root\_protect\_demo 证明：active>0 期间对象不回收、任务
  结束后收敛至 0 无泄漏）。
...
- p.6.5.10：m6\_actor 全量零回归；路线 A/B 产物不受影响；简单 vs 复杂同一程序输出/退出码一致。
...
- 平台差异（Windows 线程/同步原语）→ 沿用 kernel32/CRT 既有 extern 手法（参照 m6\_actor 既有实现）。
