...
原始 37 处，A5 已删 5 处，剩余 32 处：

| 函数 | 原始 | A5 已删 | 剩余 | 类别 |
|------|------|---------|------|------|
| tadd2 | 2 | 2 | 0 | 纯位运算（已删） |
| tmul2 | 2 | 2 | 0 | 纯位运算（已删） |
| quant3 | 1 | 1 | 0 | 纯位运算（已删） |
| rr32 / rl32 | 2 | 0 | 2 | **必要**（`<<`，n=0 边界） |
| parse_hex32 | 1 | 0 | 1 | **必要**（`<< 4`） |
| block_planes_skey | 2 | 0 | 2 | skey `*3+` **必要**；M/N 返回防御可省 |
| digest 计数器 | 4 | 0 | 4 | **必要**（`+` 进位） |
| compress | 7 | 0 | 7 | 部分可省（初始化/h 更新 `^` 32 位） |
| compress48 / fin_synth | 4 | 0 | 4 | 可省（`^`/`&` 32 位值） |
| absorb | 2 | 0 | 2 | 可省（tadd2 输出写回） |
| couple | 4 | 0 | 4 | 可省（^/| 32 位值写回） |
| lfsr_mix | 7 | 0 | 7 | 可省（^ 32 位值写回；fb 亦 32 位） |
| **合计** | **37** | **5** | **32** | 必要 ~8 / 可省 ~24 |
...
## 5. A5 落地（已完成，分支 feat/tsha1-a5-mask）

- **改动**：tadd2 删 o_pos/o_neg 掩码（2）、tmul2 删 amp/no 掩码（2）、quant3 删结果掩码（1）。
- **依据**：§2 不变量（操作数 32 位 + 纯 `&/|/^` 链位宽不增长）。
- **验证**：f/r/b/x 探针（269 项，KAT 期望值已由并行会话 cc99e5c 同步）**全 PASS 且与
  cc99e5c 逐字 identical**（Compare-Object 空）。分支隔离并行算法重构（cc99e5c）。
...
- A5-b（防御性写回掩码 ~21 处）：低优先增量，可并入 U1 后统一消除。
