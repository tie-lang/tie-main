...
**定位**：tie 平台自有通用安全哈希；**多原语混合构造**——ARX 轮（BLAKE 家族已验证结构）
为基，混入 S-盒/位切片（AES 型 GF(2^8) 非线性）+ 置换网络/SPN（Keccak/Ascon 型置换步），
软件快、状态小，与 BLAKE2 指纹底座同宗、实现可交叉验证。结构信任 BLAKE 族与所借用
标准原语分析，自创仅限参数/常量/轮数/排列组合层，不宣称新结构贡献；附带 security-notes
说明未经独立审计。

**家族（tsha1 代，四档 × 位长）**：

| 档位 | 名称 | 设计 | 速度相对纯 BLAKE |
|---|---|---|---|
| 快速 | `tsha1f-<n>` | **B 骨架**：T 轨旁路扰动（消息先经 trit 置换/扰动）+ 24 基混合调度 + BLAKE-ARX 主体 | ≈ 1.3–2.0× |
| 复杂 | `tsha1b-<n>` | **A 骨架**：双轨并行压缩（B 轨 ARX + T 轨 trit 扩散，每轮互转）；T 轨用位平面（见下）；混入 S-盒/位切片 + SPN 强化 | ≈ 2× |
| 加强 | `tsha1x-<n>` | **f 与 b 多次排列组合再计算**（对 f 与 b 的输出做多次排列组合混合再算），显著增加破解难度 | 最慢 |
| 嵌入式 | `tsha1r-<n>` | **嵌入式安全算法族**（对齐 rdu 层）：轻量 SPN（Ascon 型）+ trit 混合扰动 + 状态压缩，面向受限环境 | 轻量 |

**trit 表示（层内标准）**：**位平面（bit-sliced）**——每个 u64 存 32 个 trit，高 32 位为
幅值位平面、低 32 位为符号位平面；trit 加/旋转/混洗全部走位运算（`&/^/|/<<`），
消除逐 trit 模 3 运算的慢点。提供 pack/unpack 与 tie 原生 trit（-1/0/+1）互转原语。
**24 基自洽**：24 = 2³×3，消息字节按每 3 位配 1 trit 形成混合基数数字（0..23）参与
调度与混合。

**命名**：`tsha1<档>-<位长>`；位长对齐 128（r 档）/256/512，后续代次递增（tsha1 → tsha2 → …）。
