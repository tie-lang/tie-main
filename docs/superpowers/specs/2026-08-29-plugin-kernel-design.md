...
**命名**：`tsha1<档>-<位长>`；**位长 = 48 进制数字个数**（默认输出 48 进制），如
`tsha1f-32` = 输出 32 位 48 进制数 = 32 × log₂48 ≈ 178.7 位 ≈ 22.3 字节。可切换 16 进制
（bit 表示；`tsha1f-32-hex` 或调用参数指定 hex）。48 = 2⁴×3（16×3）为混合基数——每 4 位
二进制（16 态）配 1 trit（3 态）形成 0..47 混合基数数字，比 24（2³×3）更均匀，字符
更短（48 字符连续集合：24 位字节 → 5 个 48 进制字符 ≈ 40% 缩短 vs hex）。tsha1r 档位长
对齐 16/24/32（r 档 128 位 ≈ 23 字符）；后续代次递增（tsha1 → tsha2 → …）。
