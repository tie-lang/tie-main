...
- [x] r.1.6.6 Linux 自举 + 回归 + 打包（GitHub Actions linux-r16.yml 两条 job 完整流水线：windows 用**重建入库 stage0 tiec.exe**（r.1.6 接线后重建，含 set_target + toolchain Linux 链接修复）交叉编译 driver.tie --target linux-x64 → driver-linux.ll（CI 内置 Win32-call 门禁）→ ubuntu clang 链接（Linux CRT 就地解决 + **linux_compat.c POSIX shim（GetTickCount/_gcvt，编译器自身 Windows CRT 引用）+ trm_lite_linux.a（tl_tbl 表运行时）+ -lm（LLVM 浮点 libcall）**）→ tiec-linux 自举 tiec-linux2（--emit-ir SHA 恒等 = Linux 自举不动点；link_exe/link_shared Linux 分支补 -lm、find_trmlite_lib(target) 支持 trm_lite_linux.a）→ regress-s21 全量回归 → 打包 tie-2026.1-linux-x64.zip（捆绑 clang/opt/llvm-ar/lld + std/ext/rdu/docs）upload artifact；本地预演：driver.ll 纯净（零 Win32 call、28 POSIX call）、clang 链接仅缺 Linux CRT/crtn.o 无 datalayout 报错）
