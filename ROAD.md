...
- [ ] p.9.20.5 **t3 过程间 pass**：小函数内联、tail call、字符串构建融合（衔接 p.9.17.1 字符串原语）。**[进行中 2026-09-21，tiec 00dd649：内联内核已实现并门控关闭（t3 暂映射 t2），driver 级自举存未收敛 XREF]**
  ...
  * 未开工：tail call（IR 层无尾调用标志位，需 llvmgen 文本级识别联动）、字符串构建融合（需先勘察 p.9.17.1 字符串原语 IR 形态）。
