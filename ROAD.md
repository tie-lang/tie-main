...
- [x] p.9.3.9 **tsh REPL v1 语义修复**（p.9.21.7 实测登记，面 A 前置）：**[落地 2026-09-24，tiec cfde2de / tshell tsh_main.exe 升格：根因单一化——split_top 把顶层 var 归定义词（is_def_word），var 声明（含带副作用 init：exec_code/file_read/file_write）被提升到定义段、在 register_top_level 阶段先于 main 体全部语句求值 → 源码序破坏。D4/§4 全部玄学（「变量赋值不稳定」「exec_code 异步须 sleep」「list_dir/file_read/file_exists 首次观测被记住」）皆为该根因的表象——底座原语本身无缓存、exec_code（libc system）本就同步。修复：var 移出定义词，留在语句段按源码序执行（exec_stmt_var_decl 顶层落 globals 跨行持久；函数仍第一趟注册，声明先于调用不回退）。①②③④四点中①②为伪象（随根因消失）、③即根因、④exec_output 裸内部命令行为保留原状（脚本本约定 cmd /c 包装）。bootstrap-fp/regress 移除全部 sleep_ms workaround 后全绿；修复后 bootstrap-fp [4/4] 打印哈希与产物直核一致（8292cfa5）。探针套件（写后读/exec 后读/顺序/环境变量，fresh 场景）全过。回归 157/8/2 一致；不动点 25b9bba9 → 8292cfa5（interp 经 semantic→mexpand 进 driver 编译单元，需自举）。tshell 仓独立 commit（tsh_main.exe 重编升格）]**
...
- [x] p.9.17.1 **运行期字符串/容器原语优化**：`str_char`/逐码点遍历/拼接分配 + 容器复制遍历优化；全局受益（编译器/解释器/tsp）。**[str_char 缓存落地 2026-09-25，tiec 157f8e1：码点→字节偏移缓存重上为手写 helper 单体（llvmgen_sc.tie：双槽 @tie_sc_addr/len/n/tab 各 1MB .bss + @tie_sc_off/@tie_sc_inval；重建循环 n 存循环 phi k——phi 差一根因修正；双槽按 (addr>>3)&1 覆盖交替串对；free 路径失效钩子防 malloc 同址同长陈旧命中；CAP 兜底 legacy 步进，i<0 语义逐位保留），irgen 只发一次 call + 两个线性块（旧 opt numbering 卡点结构性消失）。验收：bench_sc_last（100K 恒取末码点最坏情形）38ms→**0ms**、bench_50k 4ms→**0ms**、bench_mb/scprobe/scalt 与基线 stdout 逐字节一致；回归 157/8/2；三阶自举 FIXED-POINT OK；不动点 19f11c25 → **0cf19245**（盐 v6）。注意：本档旧基线数字「编译路径 O(n²)、100K>120s」已失效（现基线顺序遍历 100K=19ms/恒末码点 38ms）——缓存收益集中在非顺序访问模式。容器 COW/移动语义裁定 v1 维持拷贝；str_sub_bytes 勘察结论 = 字节切片 O(len) 单次 memcpy，缓存范式不适用。详见 docs/designs/interp-performance.md §4.1.1 + tiec docs/p9216-findings.md §10/§11/§12]**
- [ ] p.9.17.2 **解释器拆箱标量**：int/float/bool/trit/char 拆箱，消灭 per-value 10 表 push；与 JIT 统一接口。**（兼任 p.9.18.1——同一解释器同一工作，本档为唯一立项处，完成时 p.9.18.1 同步勾选）**
- [ ] p.9.17.3 **优化树遍历直驱**：`exec_stmt`/`gen_expr` 直分派 switch + 解码直驱，作 JIT 冷路径基线。**（兼任 p.9.18.2——同上，不重复立项）**
...
- [ ] p.9.18.1 **面 A 拆箱 + 值池/常量池复用**：**并入 p.9.17.2 执行**（同一解释器同一工作，不重复立项）；面 A 侧附加交付 = 嵌入裁剪形态与内存峰值/稳态报告（随 p.9.18.5 验收）。
- [ ] p.9.18.2 **面 A 直驱 + 字节级规避**：**并入 p.9.17.3 执行**（同上）；面 A 侧附加交付 = 嵌入形态下低延迟指标（随 p.9.18.5 验收）。
- [ ] p.9.18.3 **面 B trm tieir-interp 嵌入面**：tieir 紧凑表示 + Backend 接口 + 统一对象身份/GC + `trm-embedded` 静态子集；源码→tiec→tieir→interp 前端执行（衔接 p.7.3.2）。**[地基已通 2026-09-24，trm 6c70b75 + f6b0e20（p.9.21.7 D2）：trm loader 直读 tiec 产出 .tir 二进制（值空间重映射 + 布局自动探测）+ interp alloca/load/store——直线纯函数 add/sub/mul 全 PASS；本项剩余 = br/cond_br/switch/call 族/表与字符串原语 + Backend 接口 + trm-embedded 子集]**
...
- [x] p.9.21.8 **片段池过滤 + 模块缓存默认开**：**[落地 2026-09-24，tiec 5e93660：write_mod_slice 引用收集式池压缩——引用来源 = 函数名/块名、符号表/导出选中行 name+sig、操作数 kind 3（全局名）、kind 0 池载荷操作数①（const_str(62)/const_f(51)/call(35)/extern_call(36)/call_vararg(41)，与 llvmgen collect_strings/collect_externs 清单对齐；ROAD 原文「kind=2」为笔误，实际 kind=0+特定 opcode）→ 池保序压缩 + 写侧全量重映射（未标记 id 响亮失败）；deserialize 池重建时构建文件 id → 进程 id 映射，段 4/5/6 池引用统一转换（全量池主单元恒等，D1 roundtrip 零影响）；顺手修复池载荷操作数①被误值平移的潜伏缺陷（含字符串字面量模块片段写入错位）。摘 TIEC_MODCACHE 门控默认开。tieir_test 补 const_str/全局名池重映射断言。数据：全量池写放大 72.9s → 44.4s（片段写增量 42.4s → 13.5s，-68%）；--no-cache 全量 30.9s vs 旧版 30.5s 持平（基线 26.8s 为旧环境记录）；模块缓存确定性在 .tir 产物层逐字节一致（exe 层链接器非确定为既有现象，与缓存/改动无关——旧编译器同款复现）。四步验收：冷启 +4 / 重编 +0 / 叶子改动恰 +1 / .tir SHA256 一致。不动点 7b7d8886 → 25b9bba9，回归 157/8/2 一致。备注：bootstrap-fp [4/4] 打印哈希又现陈旧值（tsh exec_output 语义缺陷复现，产物直核为准），归 p.9.3.9]**
