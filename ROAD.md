...
- [ ] p.9.3.9 **tsh REPL v1 语义修复**（p.9.21.7 实测登记，面 A 前置）：①exec_code 改同步等待（或提供 exec_code_wait）——现行异步返回导致脚本读不到子进程产物；②去除 list_dir/file_read/file_exists 的进程内按路径缓存（同路径重复观测返回陈旧列表/伪值）；③脚本内变量赋值稳定性——同轮运行中不同语句对同一变量可能见到不同值（bootstrap-fp 实测：哨兵见默认目录、编译见参数目录、[4/4] 又见默认；先写最小复现探针再修，若根因在求值器深层允许降级为文档化安全范式并如实登记）；④exec_output 对裸内部命令返回空（自动包装或显式报错）。验收：bootstrap-fp/regress 移除 workaround sleep 后全绿 + tshell 仓独立回归（tiec 三阶自举不受影响——tshell 不进 driver 编译单元）。
