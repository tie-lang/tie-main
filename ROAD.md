...
- [x] p.9.21.3 **driver 全拆 + 批量拆分**：>800 行文件逐文件子任务化，全仓 ≤800（gen 豁免）。**[已落地 2026-09-23，收口态见下]**
  ...
  * 工具（**已入库** `tiec/tools/` + `tiec/tools/split/`，换机可用）：`split_parts.py`（命名空间感知整函数分片 + 完整性校验）、`split_dispatch.py`（整数调度器分支提取）、`split_branch_blocks.py`（语句级分支提取）、`split_dispatch_segments.py`（哨兵分派链分段）、`split_builtin_branches.py` / `split_expr_files.py` / `split_driver.py`（历史范式记录）、`func_len.py`（函数行数审计）、`orphan_check.py`（孤儿源码检查）、`fp.sh`（三阶自举不动点）、`repair_objects.py`（odb 回填，哈希校验）。
...
- [ ] p.9.21.4 **II1 强制可见性**：namespace 内非 pub 跨 ns 不可见（先诊断后强制），tiec dogfood。**[诊断步已落地 2026-09-23，tiec d5fe664 + tie-main 设计 §II1 现状核实]**
  ...
  * 全仓普查（`tiec/tools/visibility_survey.py`）：59 个 ns、2415 个 ns 内函数（pub 1261 / 私有 1154，48% 私有）、141 个顶层函数豁免。
