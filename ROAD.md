...
- [ ] p.8.1.6 表删除/缩表原语：动态表 `pop` / 截断缩表（现仅 `table_push` + 下标写，无删除/缩表，删除靠重建表模拟）——**尽早修复（tge ECS 实体/组件删除支撑前置）**，2026-09-12 查证确认缺口
- [ ] p.8.1.7 enum payload 白名单扩展：放开 `table/f64` payload（现白名单排除；联动 p.8.1.4 结构化解构）——**尽早修复（ECS 组件标记/事件类型支撑前置）**，2026-09-12 查证确认缺口
