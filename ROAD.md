...
- [ ] p.9.17.1 **运行期字符串/容器原语优化**：`str_char`/逐码点遍历/拼接分配 + 容器复制遍历优化；全局受益（编译器/解释器/tsp）。**[中期 2026-09-24/25：str_char 单槽缓存方案已验证（50K 遍历 9.2s→0.27s）后回滚，遗留 phi 退出值差一 + 交替串单槽失效两缺陷；str_sub_bytes 勘察结论 = 字节切片 O(len) 单次 memcpy 无 O(n²)、缓存范式不适用；容器 COW/移动语义裁定 v1 维持拷贝。详见 docs/designs/interp-performance.md §4.1.1 + tiec docs/p9216-findings.md §10/§11]**
