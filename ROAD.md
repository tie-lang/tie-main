...
- [ ] p.9.21.3 **driver 全拆 + 批量拆分**：>800 行文件逐文件子任务化，全仓 ≤800（gen 豁免）。**[进度 33 → 3 个超限文件（降 91%），余量见下]**
  * 已完成（tiec ff4682b / 5111ea9 / bcec2aa / bbf64d7 / e49d24e / 1f1137b）：driver 库 0 超限（4392 行 / 18 文件）；`irgen_expr` 8790 → 744 + 16 域文件；后端 8 文件整函数分片 + `irgen_stmt`/`llvmgen_inst` 两大调度器分支提取（`tig_stmt` 1026 → 32、`gen_inst` 769 → 191）；前端/解释器/配置 12 文件分片（含命名空间感知分片，见下）；`sbuiltin.builtin_call` 1122 → 165（93 个 `fn_name ==` 分支提取）。
  * 现状（2026-09-22 实测，不含 gen 豁免与 proto 豁免）：**3 个文件超 800 行** —— `frontend/sstate.tie` 1919（179 个顶层全局数据表）、`frontend/sinfer_q2_q1.tie` 1522（单函数 `infer_expr` 1523 行）、`frontend/scheck_q2_q1.tie` 926（单函数 `check_stmt` 927 行）。**已全清的库**：driver / trm / ir / interner / columnar / ast / tieir / core / config / lex / parse / llvmgen / interp / diag / types / passes / sema（irgen 仅剩 sstate 属 state 类）。
  * 余量三类：①**非分派型巨型函数**（`infer_expr` / `check_stmt`，需语句级提取而非分支提取，属独立子项）；②**数据主导文件**（`sstate.tie` 余量是 179 个全局表，需把表初始化搬进函数或将表拆到多文件——全局 var 允许分布在多个文件顶层）；③gen 文件豁免（`diagcode_cat.gen`）。
  * 单函数超 300 行（D4 余量）：`infer_expr` 1523、`check_stmt` 927、`inline_expand` 414、`tig_parse_float` 358、`tig_inflate_raw` 541。
  * **拆分工程铁律（实测踩坑，工具已固化）**：①else-if 链/多行条件块必须整块搬（含起始 `if` 与闭合 `}`）；②链块与相邻语句共享局部变量时按「区域」搬或就地保留；③花括号计数必须字符串/注释感知；④拆出文件不含 import 与顶层 var；⑤`main` 必须留顶层（ns 内缺入口 LNK1561）；⑥**命名空间归属**：`namespace X {` 之前的函数是顶层函数，跨 ns 裸调依赖其顶层身份（semantic 的 `sm_warn_*` 被 `namespace scheck` 裸调）——分片必须按各自上下文包裹；⑦**分片文件绝不覆盖**（曾覆盖 `putil_p1` 导致 `split_current_gt` 定义丢失）→ 新分片用 `_qN` 后缀 + 函数集完整性校验；⑧void 调度分支须多行体（tie 语句以换行分隔）。
  * 工具（可复用，均在 `F:\Projects\tie-repo\_tiec_verify\`，非入库）：`split_driver.py`（driver 分域 + 限定名）、`split_bi3.py`（builtin 分支提取）、`split_expr_files.py`（按域搬函数）、`split_parts5.py`（命名空间感知整函数分片 + 完整性校验）、`split_dispatch.py`（tag/opcode/字符串/多行条件调度器分支提取 + 形参透传）、`fp.sh`（三阶自举不动点）。
  * 附带发现（待清理）：`compiler/middle/pass/*` 孤儿模块；`frontend/putil.tie` 的 `expend_type_gt` 系死代码但引用链完整，`split_current_gt` 定义随覆盖事故暴露过一次、已随 `_qN` 规则修复。
