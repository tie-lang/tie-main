...
- [ ] p.9.21.7 **tieir 反序列化布局忠实重建（p.9.21.6 自检发现）**：deserialize 的 fpo 校验假设参数段连续，真实单元为每函数 [参数][操作数] 交错 → 自产多函数单元无法过自身 tieir.read（复现 _tiec_verify/rr_probe.tie）。修法 = 反序列化值重映射（与 tieir_slice.write_mod_slice 对偶）+ 多函数 roundtrip 自检；解开后 trm loader 改直读二进制、片段组装消费（改叶子只重编该模块）接续。
