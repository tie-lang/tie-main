...
- [ ] p.9.21.8 **片段池过滤 + 模块缓存默认开**：write_mod_slice 引用收集式池压缩（const_str 的 kind=2 操作数①、kind=3 全局名、函数名/块名/符号/导出/依赖行 → 池保序压缩 + id 重映射），消除全量池写放大（实测 27s→68s）；摘掉 TIEC_MODCACHE 门控默认开。验收：driver 自编基准回到 ≈27s（±10%）+ verify-modcache 四步验收语义不变 + tieir_test 片段 roundtrip 补 const_str/全局名覆盖仍绿 + 自举升格回归 157/8/2。
