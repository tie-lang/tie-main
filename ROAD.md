...
- [ ] p.9.3.7 tsh 脚本运行时补全（0-Rust 自举后残留缺陷/能力清单，2026-09-13 实测登记）：①interp 函数内多局部变量与 exec_* 内建共存缺陷（局部槽丢失）②递归 re-entrancy 残边（递归调用后两 var 场景；活动段计数已修未完全）③file_exists/mkdir_all 运行期新建路径陈旧伪值（需盘上实查）④字节数组/substr 二进制读写 ⑤进程 stdin/stdout 交互管道（解锁 verify-tiedap）⑥exec_code_timeout 真超时强杀 ⑦zip/递归筛选复制/文件大小/求和 ⑧高分辨率单调时钟（time_now 秒级）⑨regex `\|` 与 `-replace` 语义 ⑩var 偶发 command-not-found 扰行；验收=剩余 .ps1（package×2 / verify-tiedap）改写完成，grep .ps1=0
